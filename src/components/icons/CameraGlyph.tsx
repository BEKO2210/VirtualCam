...
/**
 * Standalone camera body mini-render for tile cards.
 * Identical geometry rules as the hero rig — same body silhouette engine —
 * but rendered without a lens, so the body shape is what reads.
 */
export function CameraGlyph({ camera, format, accent = 'var(--color-primary)', className }: Props) {
  const body = bodyGeometry(camera, format);

  // Center the body in the viewBox.
  const bodyX = (VIEW_W - body.w) / 2 + 4;
  const bodyY = (VIEW_H - body.h) / 2 + 4;
  const cx = bodyX + body.w * 0.32 + 26; // approx lens-mount center
  ...
  return (
    ...
    >
      ...
      {/* Lens stub (so the body doesn't look amputated) */}
      <g>
        <rect
          x={cx - 16}
          y={cy - 14}
  ...
  return (
    <g>
      ...
      {body.silhouette !== 'cinema' && (
        <>
          <circle
            cx={right - body.gripW - 14}
            cy={y + 6}
            r={5}
            fill="oklch(0.42 0.012 270)"
            stroke={accent}
            strokeWidth="0.8"
            opacity="0.9"
          />
